...
const PLATFORM = 'web';
const {token:userAuthToken} = JSON.parse(localStorage.getItem(USER_TOKEN)) || {};

const authSocket = io(`${url}/auth`);
const tasksSocket = io(`${url}/task`, {query:{ token: userAuthToken}});
...
// authSocket.on(EVENTS.error, (err)=>{
//     console.log("Auth socket Error", err);
// })
...
export const handleSignOut = (cb)=>{
    // Reconnect auth socket
    authSocket.connect();
    // disconnect task socket
    tasksSocket.disconnect();
    // in callback, delete token, and reset username and todos
