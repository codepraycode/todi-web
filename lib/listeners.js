// Handle actions taken on events

// Authentication Listeners

import { renderTodosError } from "./renderer";
...
export const listenForTodosConnectionStatus = (connecting=true)=>{
    ...
    if (connecting) document.querySelector(".task-items").setAttribute('data-loading', 'true');
        ...
        const {username} = JSON.parse(localStorage.getItem(USER_TOKEN)) || {};
        document.getElementById("auth").innerText = username || 'annonymous';
...
export const listenForTodosConnectionFailed = (error, signOutHandler)=>{
    if (error === 'auth_error') {
        renderTodosError("Authentication is required!");
